...
class User
  ...
  def initialize(id:, first_name:, last_name:, email:, company_id:,
    ...
    raise ArgumentError, 'User first_name must not be empty' if first_name.strip.empty?
    raise ArgumentError, 'User last_name must be a String' unless last_name.is_a?(String)
    raise ArgumentError, 'User last_name must not be empty' if last_name.strip.empty?
    raise ArgumentError, 'User email must be a String' unless email.is_a?(String)
    raise ArgumentError, 'User email must not be empty' if email.strip.empty?
